...
const UploadTrackDialog = ({ trigger }: UploadTrackDialogProps) => {
  ...
  const { uploadTrack, uploading } = useTracks();
  const { user } = useAuth();
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) resetForm(); }}>
      ...
      <DialogContent className="bg-surface-elevated border-border max-w-md">
        <DialogHeader>
          <DialogTitle className="text-foreground">Upload Track</DialogTitle>
          <DialogDescription className="text-subdued">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {uploading && (
            <div className="space-y-2">
              <p className="text-sm text-subdued">Uploading...</p>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button
              type="submit"
              disabled={!audioFile || !title.trim() || !artist.trim() || uploading}
